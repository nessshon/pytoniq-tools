from __future__ import annotations

from typing import Optional, List, Tuple, Union

from pytoniq_core import Cell, WalletMessage

from .._base import Wallet
from ..data import WalletV4Data
from ...client import Client


class WalletV4R1(Wallet):
    """
    A class representing Wallet V4 R1 in the TON blockchain.
    """

    CODE_HEX = ""  # noqa

    @classmethod
    def create(cls) -> Tuple[WalletV4R1, bytes, bytes, List[str]]:
        """
        Create a new WalletV4R1 instance.

        :return: A tuple containing the wallet instance, public key, private key, and mnemonic phrase.
        """
        return super().create()  # type: ignore

    @classmethod
    def from_mnemonic(
            cls,
            mnemonic: Union[List[str], str],
            client: Optional[Client] = None,
    ) -> Tuple[WalletV4R1, bytes, bytes, List[str]]:
        """
        Create a WalletV4R1 instance from a mnemonic.

        :param mnemonic: The mnemonic phrase.
        :param client: The client to interact with the blockchain.
        :return: A tuple containing the wallet instance, public key, private key, and mnemonic phrase.
        """
        return super().from_mnemonic(mnemonic, client)  # type: ignore

    @classmethod
    def _create_data(cls, public_key: bytes, wallet_id: int) -> WalletV4Data:  # noqa
        """
        Create wallet data for WalletV4R1.

        :param public_key: The public key of the wallet.
        :param wallet_id: The wallet ID.
        :return: An instance of WalletV4Data.
        """
        return WalletV4Data(public_key=public_key)

    @classmethod
    def _raw_create_transfer_msg(
            cls,
            private_key: bytes,
            messages: List[WalletMessage],
            seqno: int = 0,
            wallet_id: int = 698983191,
            valid_until: Optional[int] = None,
            op_code: Optional[int] = 0,
    ) -> Cell:
        """
        Create a raw transfer message for WalletV4R1.

        :param private_key: The private key of the wallet.
        :param messages: The list of wallet messages.
        :param seqno: The sequence number.
        :param wallet_id: The wallet ID.
        :param valid_until: The validity timestamp.
        :param op_code: The operation code.
        """
        return super()._raw_create_transfer_msg(
            private_key=private_key,
            messages=messages,
            seqno=seqno,
            wallet_id=wallet_id,
            valid_until=valid_until,
            op_code=op_code,
        )


class WalletV4R2(WalletV4R1):
    """
    A class representing Wallet V4 R2 in the TON blockchain.
    """

    CODE_HEX = ""  # noqa

    @classmethod
    def create(cls) -> Tuple[WalletV4R2, bytes, bytes, List[str]]:
        """
        Create a new WalletV4R2 instance.

        :return: A tuple containing the wallet instance, public key, private key, and mnemonic phrase.
        """
        return super().create()  # type: ignore

    @classmethod
    def from_mnemonic(
            cls,
            mnemonic: Union[List[str], str],
            client: Optional[Client] = None,
    ) -> Tuple[WalletV4R2, bytes, bytes, List[str]]:
        """
        Create a WalletV4R2 instance from a mnemonic.

        :param mnemonic: The mnemonic phrase.
        :param client: The client to interact with the blockchain.
        :return: A tuple containing the wallet instance, public key, private key, and mnemonic phrase.
        """
        return super().from_mnemonic(mnemonic, client)  # type: ignore
